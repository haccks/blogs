...
+ Familiarity with [Qt for Python](https://doc.qt.io/qtforpython-6/) (PySide6 or PyQt5)
...
So far so good. Let's add one more functionality to our small app, a cancel button. This will give users an option to cancel the download in between. First let's modify the first example `demo-subclass.py`{: .filepath}
. If possible, we will try to achieve the same for second example! 
...
class MainWindow(QMainWindow):
    ...
    def download(self):
        ...
        self.cancelled.connect(self.worker_thread.set_is_cancel)
        self.worker_thread.start()
...
[doc](https://doc.qt.io/qt-6/qthread.html#details) says for subclass approach  
...
While download is in progress and managed by another thread, when we press `Cancel` button then the slot `set_is_cancel()` will execute in the main thread. Both of these jobs will be done in parallel. 
...
Therefore, slot `set_is_cancel()` will execute in the new thread (same thread as of the `do_work()`). Since `do_work()` is a long job, the local event lopp will be busy till this job finishes and all the incoming signals will be queued.
